...
| **資金費率** | Funding Rate | 永續合約的資金費率，通常每 8 小時結算一次 | "查詢所有交易所的即時資金費率" |
| **交易對** | Symbol / Trading Pair | 加密貨幣交易對，如 BTC/USDT | "選擇 BTC/USDT 交易對" |
| **交易所** | Exchange | 本平台支持的加密貨幣交易所，包含 Binance、OKX、MEXC、Gate.io | "在 4 個交易所之間尋找套利機會" |
| **最佳套利對** | Best Arbitrage Pair | 系統從所有交易所組合中計算出費率差異最大的兩個交易所 | "系統標示 MEXC (BUY) 和 Binance (SELL) 為最佳套利對" |
...
### User Story 2.5 - 多交易所多交易對資金費率即時監控 (Priority: P2.5)

用戶可以在 Web 界面上看到多個交易對的即時資金費率監控，**同時顯示 4 個交易所**（Binance、OKX、MEXC、Gate.io）的費率和價格數據，系統自動計算並標示最佳套利對（做多和做空的交易所）。這讓用戶能夠全面掌握市場動態，一眼看到最優的套利機會。

**Why this priority**: 這是套利機會監控的核心功能，提供更全面的市場視圖。相較於只顯示兩個交易所，4 個交易所能提供更多套利組合（C(4,2) = 6 種組合），幫助用戶找到最佳機會。系統會自動計算並標示出費率差異最大的交易所對，用戶無需手動比較。

**Independent Test**: 可以透過登入後，進入「市場監控」頁面查看表格形式的多交易對即時數據。測試成功標準是用戶能看到至少 10 個交易對的即時更新，每個交易對顯示 4 個交易所的費率和價格，並清楚標示 BUY/SELL 標籤，數據每 30 秒自動刷新。

**Acceptance Scenarios**:

1. **Given** 用戶已登入, **When** 進入「市場監控」頁面, **Then** 看到表格形式的多交易對監控面板，顯示所有配置的交易對（例如 top10 群組的 10 個交易對）

2. **Given** 用戶在「市場監控」頁面, **When** 查看表格, **Then** 每一行顯示一個交易對的完整資訊，包含 4 個交易所的獨立欄位：
   - 交易對名稱（BTCUSDT）
   - **Binance 欄位**：資金費率（第一行）、即時價格（第二行）、BUY/SELL 標籤（如果是最佳套利對）
   - **OKX 欄位**：資金費率（第一行）、即時價格（第二行）、BUY/SELL 標籤（如果是最佳套利對）
   - **MEXC 欄位**：資金費率（第一行）、即時價格（第二行）、BUY/SELL 標籤（如果是最佳套利對）
   - **Gate.io 欄位**：資金費率（第一行）、即時價格（第二行）、BUY/SELL 標籤（如果是最佳套利對）
   - 狀態指示（🔔 機會 / ⚠️ 接近 / ➖ 正常）

3. **Given** 用戶查看某個交易對的費率, **When** 系統計算出最佳套利對, **Then** 在費率最低的交易所標記「BUY」（綠色背景），在費率最高的交易所標記「SELL」（紅色背景）
   - 例如：BTCUSDT 的費率為 Binance 0.08%, OKX 0.02%, MEXC -0.01%, Gate.io 0.05%
   - 系統會在 MEXC 標記「BUY」（費率最低），在 Binance 標記「SELL」（費率最高）

4. **Given** 用戶在「市場監控」頁面, **When** 系統檢測到費率更新, **Then** 表格數據自動更新（每 30 秒），無需手動刷新

5. **Given** 表格中某個交易對達到套利閾值, **When** 最大費率差異 ≥ 0.5%, **Then** 該行以高亮顯示並標記狀態為「🔔 機會」

6. **Given** 表格中某個交易對接近閾值, **When** 最大費率差異在 0.4%-0.5% 之間, **Then** 該行標記狀態為「⚠️ 接近」

7. **Given** 用戶想自訂監控的交易對, **When** 點擊「配置交易對」按鈕, **Then** 可以選擇預設群組（top10, top20, defi 等）或自訂交易對列表

8. **Given** 用戶在表格中點擊某個交易對, **When** 查看詳細資訊, **Then** 顯示該交易對在所有 4 個交易所的歷史費率走勢圖和詳細數據

9. **Given** 用戶想快速開倉, **When** 在表格中點擊「快速開倉」按鈕（僅限機會行）, **Then** 直接進入開倉對話框，預設選擇標記 BUY 和 SELL 的交易所

10. **Given** 某個交易所的 API 暫時失效, **When** 系統無法獲取該交易所的數據, **Then** 該交易所欄位顯示「--」或「N/A」，並繼續顯示其他 3 個交易所的數據，套利計算基於可用的交易所
...
#### 多交易對即時監控
- **FR-025-1**: 系統必須提供多交易對即時監控頁面，以表格形式同時顯示多個交易對的資金費率數據（類似 CLI 表格模式）
- **FR-025-2**: 系統必須允許用戶選擇要監控的交易對群組：
  - 預設群組：top10（市值前 10）、top20（市值前 20）、defi、layer1、meme
  - 自訂交易對列表：用戶可手動輸入或選擇交易對
  - 群組配置來源：從 `config/symbols.json` 載入預設群組
- **FR-025-3**: 多交易對監控表格必須顯示每個交易對的以下資訊：
  - 交易對名稱（BTCUSDT）
  - Binance 資金費率（百分比，保留 4 位小數）
  - OKX 資金費率（百分比，保留 4 位小數）
  - 費率差異（絕對值，百分比）
  - 預期年化收益率（百分比）
  - 狀態指示：
    * 🔔 **機會**（差異 ≥ 0.5%）- 綠色高亮
    * ⚠️ **接近**（差異 0.4%-0.5%）- 黃色警告
    * ➖ **正常**（差異 < 0.4%）- 灰色
  - Binance 價格（用於參考）
  - OKX 價格（用於參考）
  - 價差百分比（用於判斷價差方向）
  - 最後更新時間
- **FR-025-4**: 系統必須透過 WebSocket 即時推送多交易對的費率更新：
  - 事件類型：`rates:update`（批量更新）
  - 推送頻率：每 5 秒推送一次所有交易對的最新數據
  - 事件 payload 包含：交易對數組，每個包含完整的費率和價格資訊
  - 增量更新：僅推送有變化的交易對數據
- **FR-025-5**: 系統必須支援表格排序和篩選：
  - 按費率差異排序（預設）
  - 按年化收益率排序
  - 按交易對名稱排序
  - 只顯示「機會」狀態的交易對
  - 只顯示「接近」或以上狀態的交易對
- **FR-025-6**: 系統必須支援從監控表格直接開倉：
  - 每行顯示「快速開倉」按鈕（僅限「機會」狀態）
  - 點擊後直接進入開倉對話框，預填交易對和方向資訊
  - 無需切換到「套利機會」頁面
- **FR-025-7**: 系統必須支援點擊交易對查看詳細資訊：
  - 顯示該交易對的歷史費率走勢圖（過去 24 小時）
  - 顯示歷史最高/最低費率差異
  - 顯示該交易對的歷史套利機會記錄
- **FR-025-8**: 系統必須提供即時統計摘要（顯示在表格上方）：
  - 正在監控的交易對總數
  - 當前機會數量（差異 ≥ 0.5%）
  - 接近閾值數量（差異 0.4%-0.5%）
  - 最高費率差異（顯示交易對和數值）
...
- **並發開倉範圍澄清**: FR-034 定義了「防止同一用戶快速多次點擊同一開倉按鈕」的並發控制。補充說明：系統允許用戶對**不同交易對**並發開倉（例如同時開 BTCUSDT 和 ETHUSDT），但對**同一交易對**同時只能有一個開倉操作進行中。實作方式：使用 Redis 分散式鎖，鎖的 key 格式為 `open-position:{userId}:{symbol}`，確保同一用戶對同一交易對的開倉操作串行執行
- **數據保留政策**:
  - **套利機會記錄**：ACTIVE 狀態的機會實時更新，EXPIRED 狀態的機會保留 7 天後自動刪除（用於短期分析）
  - **交易歷史記錄**：永久保留所有已平倉交易記錄（Trade 表），不自動刪除
  - **持倉記錄**：未平倉的 Position 記錄永久保留，直到用戶手動平倉或標記為異常
  - **審計日誌**：保留 90 天，90 天後可選擇性歸檔到冷儲存（如 S3）或刪除
  - **通知日誌**：保留 30 天後自動刪除
- **系統降級策略細節**（擴展 SC-012）：
  - **Binance API 故障**：暫停涉及 Binance 的套利機會偵測，僅顯示 OKX 相關數據；已開倉涉及 Binance 的持倉標記為「高風險」並建議用戶手動處理
  - **OKX API 故障**：暫停涉及 OKX 的套利機會偵測，僅顯示 Binance 相關數據；已開倉涉及 OKX 的持倉標記為「高風險」並建議用戶手動處理
  - **資料庫連線故障**：停止所有寫入操作，顯示「系統維護中」訊息給用戶；已登入用戶可繼續查看快取數據（唯讀模式）
  - **WebSocket 服務故障**：自動降級為輪詢模式（每 5 秒 HTTP 請求一次），並顯示「即時推送暫時不可用，已切換為輪詢模式」提示
  - **Redis 故障（分散式鎖）**：降級為單機鎖（使用內存），並限制並發用戶數至 5 人，超過限制時顯示「系統繁忙」訊息
