...
### User Story 2.5 - 多交易對資金費率即時監控 (Priority: P2.5)

用戶可以在 Web 界面上看到多個交易對（類似 CLI 表格模式）的即時資金費率監控，顯示所有監控交易對的費率差異，無論是否達到套利閾值。這讓用戶能夠全面掌握市場動態，觀察潛在機會的形成。

**Why this priority**: 這是套利機會監控的補充功能，提供更全面的市場視圖。用戶可以看到所有交易對的費率變化，包括尚未達到閾值但正在接近的潛在機會。依賴 P2 的基礎監控功能，但提供了更豐富的數據展示。

**Independent Test**: 可以透過登入後，進入「市場監控」頁面查看表格形式的多交易對即時數據。測試成功標準是用戶能看到至少 10 個交易對的即時更新，數據每 5 秒自動刷新。

**Acceptance Scenarios**:

1. **Given** 用戶已登入, **When** 進入「市場監控」頁面, **Then** 看到表格形式的多交易對監控面板，顯示所有配置的交易對（例如 top10 群組的 10 個交易對）
2. **Given** 用戶在「市場監控」頁面, **When** 查看表格, **Then** 每一行顯示一個交易對的完整資訊：
   - 交易對名稱（BTCUSDT）
   - Binance 資金費率
   - OKX 資金費率
   - 費率差異（百分比）
   - 預期年化收益率
   - 狀態指示（🔔 機會 / ⚠️ 接近 / ➖ 正常）
3. **Given** 用戶在「市場監控」頁面, **When** 系統檢測到費率更新, **Then** 表格數據自動更新，無需手動刷新
4. **Given** 表格中某個交易對達到套利閾值, **When** 費率差異 > 0.5%, **Then** 該行以高亮顯示並標記「🔔 機會」
5. **Given** 表格中某個交易對接近閾值, **When** 費率差異在 0.4%-0.5% 之間, **Then** 該行以警告色顯示並標記「⚠️ 接近」
6. **Given** 用戶想自訂監控的交易對, **When** 點擊「配置交易對」按鈕, **Then** 可以選擇預設群組（top10, top20, defi 等）或自訂交易對列表
7. **Given** 用戶在表格中點擊某個交易對, **When** 查看詳細資訊, **Then** 顯示該交易對的歷史費率走勢圖和詳細數據
8. **Given** 用戶想快速開倉, **When** 在表格中點擊「快速開倉」按鈕（僅限機會行）, **Then** 直接進入開倉對話框，無需切換到「套利機會」頁面
