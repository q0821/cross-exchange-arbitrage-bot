...
- **FR-020**: 系統必須識別當費率差異超過設定門檻時的套利機會（預設門檻：0.37%）。

  **成本結構說明**（基於一般用戶等級，以 $100,000 倉位為例）：
  - 手續費：0.2%（開倉 0.1% + 平倉 0.1%，Taker Fee 0.05% × 4 次操作）
  - 滑點：0.1%（市價單保守估計，流動性較差時可能更高）
  - 價差成本：0.05%（兩交易所標記價格差異）
  - 安全邊際：0.02%（預留緩衝應對市場波動）
  - **總成本：0.37%**（$370）

  **獲利邏輯**：
  - 資金費率差 = 0.37%，單次結算收益 = $370，扣除成本後**剛好打平**
  - 資金費率差 = 0.40%，單次結算收益 = $400，扣除成本後**淨賺 $30（0.03%）**
  - **策略優勢**：即使只持倉 8 小時（1 次結算）也能確保不虧損，不依賴資金費率持續性

- **FR-020-1**: 系統必須計算並顯示套利機會的預期淨收益。計算邏輯如下：
  - **預期淨收益** = 資金費率差 - 總成本（0.37%）
  - **預期淨收益率（年化）** = (資金費率差 - 0.37%) × (24 / 8) × 365 = (資金費率差 - 0.37%) × 1095
  - **有效機會判定**：僅當預期淨收益 > 0 時，才視為有效套利機會
  - **範例**：
    * 資金費率差 = 0.40%，預期淨收益 = 0.03%，年化淨收益率 = 32.85%
    * 資金費率差 = 0.50%，預期淨收益 = 0.13%，年化淨收益率 = 142.35%
...
- **FR-024**: 系統必須顯示每個套利機會的以下資訊：
  ...
  - 費率差異（絕對值和百分比）
  - **成本明細**：
    * 總成本：0.37%（手續費 0.2% + 滑點 0.1% + 價差 0.05% + 安全邊際 0.02%）
  - **預期淨收益**：資金費率差 - 總成本（例如：0.40% - 0.37% = 0.03%）
  - **預期淨年化收益率**：基於淨收益計算的年化報酬（例如：0.03% × 1095 = 32.85%）
  - 機會發現時間
