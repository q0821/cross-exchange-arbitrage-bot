...
- **FR-011**: 系統必須加密儲存所有 API Key 和 Secret（靜態加密，encryption at rest）
...
- **FR-020**: 系統必須識別當費率差異超過設定門檻時的套利機會（預設門檻：0.05%）
- **FR-021**: 系統必須計算每個套利機會的預期年化收益率（考慮資金費率結算頻率）
- **FR-022**: 系統必須透過 WebSocket 即時推送套利機會更新到已登入用戶的瀏覽器
...
- **FR-031**: 系統必須使用交易所支援的訂單類型（市價單或限價單）來確保執行成功率
- **FR-032**: 系統必須記錄每次開倉的詳細資訊：
  ...
  - 開倉時間
  - 開倉時的資金費率
- **FR-033**: 系統必須在開倉失敗時向用戶顯示明確的錯誤訊息，包含失敗原因
- **FR-034**: 系統必須處理部分開倉失敗的情況（一個交易所成功，一個失敗）：
  - 自動嘗試平倉已成功的部分（最佳努力，best effort）
  - 記錄異常狀態
  - 通知用戶手動處理
...
1. **API Key 洩漏**: 如果 API Key 加密不當或系統被入侵，用戶的 API Key 可能洩漏，導致資金損失
   - **緩解策略**: 使用業界標準的加密算法（AES-256-GCM）、定期安全審計、限制 API Key 權限（僅交易和查詢）
