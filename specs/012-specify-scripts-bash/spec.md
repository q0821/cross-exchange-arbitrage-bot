...
2. **Given** 交易者使用預設手續費率（每筆交易 Taker 0.05%，long 和 short 各開平倉共 4 筆，總計 0.2%），**When** 系統計算淨收益，**Then** 淨收益 = 費率差收益 - 總手續費成本（0.2%）
3. **Given** 某套利機會的費率差為 0.15%/8h，但總手續費為 0.2%，**When** 系統計算淨收益，**Then** 系統顯示淨收益為負值（-0.05%），並清楚標示此機會不適合套利
...
- **FR-005**: 系統必須使用以下公式計算淨收益：淨收益 = 費率差收益 - 總手續費成本（long 交易所開平倉 + short 交易所開平倉，共 4 筆交易，不考慮滑點和其他成本）
- **FR-006**: 系統必須允許用戶自定義手續費率假設（預設值：每筆交易使用 Taker 費率 0.05%，long 和 short 各 2 筆，總計 0.2%）
- **FR-007**: 系統必須在顯示淨收益時同時顯示計算依據（費率差、總手續費 4 筆交易的詳細數值）
...
- **淨收益計算參數**: 用於計算淨收益的各項參數，包含費率差、總手續費率（long 和 short 各建倉和平倉，共 4 筆交易，每筆使用 Taker 費率）
...
- 假設手續費率預設為每筆交易使用 Taker 0.05%，套利涉及 4 筆交易（long 建倉、long 平倉、short 建倉、short 平倉），總計 0.2%
- 假設交易者使用市價單開倉和平倉（Taker 費率），不考慮限價單（Maker 費率）的情況
- 假設 long 交易所和 short 交易所的手續費率相同（均為 Taker 0.05%）
