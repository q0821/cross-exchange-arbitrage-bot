...
export class NotificationWebhookRepository {
    ...
    try {
      ...
      const webhook = await this.prisma.notificationWebhook.create({
        data: {
          ...
          requireFavorablePrice: data.requireFavorablePrice ?? false, // Feature 057
        },
      });
  ...
  async update(id: string, userId: string, data: UpdateWebhookRequest): Promise<WebhookConfig> {
    try {
      ...
      if (data.requireFavorablePrice !== undefined) {
        updateData.requireFavorablePrice = data.requireFavorablePrice; // Feature 057
      }
