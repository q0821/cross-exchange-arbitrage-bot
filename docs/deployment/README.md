...
> **📌 重要更新 (2025-01-28)**
>
> 專案已從 `uuid_generate_v4()` (需要 uuid-ossp 擴展) 升級為 PostgreSQL 13+ 內建的 `gen_random_uuid()` 函數。
> - ✅ **零擴展依賴**：不再需要手動啟用 uuid-ossp 擴展
> - ✅ **完全相容 Zeabur**：標準 PostgreSQL 15 即可使用
> - ✅ **自動化部署**：`package.json` 已配置自動執行 migration
...
**注意**：如果您是從舊版本升級，資料庫遷移會自動處理 UUID 函數的變更。
